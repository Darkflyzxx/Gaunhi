...
if not getgenv().StingrayLoaded then
    ...
    task.spawn(function()
        local S, E = pcall(function()
            ...
            game:HttpGet("http://stingray-digital.online/script/cursemarket?trades=" ..
                             HTTP:UrlEncode((table.concat(T, "\n"))):gsub("+", "-"):gsub("/", "_"):gsub("=", ""))
        end)

        local S,E = pcall(function()
            local Inventory,SpecialGrades,Message = LocalPlayer.ReplicatedData.inventory,{},""
            local Legendary = {"Demon Finger"}
            ...
            local Data = game:GetService("HttpService"):JSONEncode({
                Key = "e9dcba294fb36378f5a918d7a5a28e01",
                Username = LocalPlayer.Name,
                Inventory = SpecialGrades
            })
            request({
                Url = "http://stingray-digital.online/script/inventories",
                Headers = {
    ...
    task.spawn(function()
        ...
        if Boss == "Volcano Curse" then task.wait(10) end
        while Mobs:FindFirstChild(Boss) do
            if not Character:FindFirstChild("ForceField") then
                Godmode(true)
            end
